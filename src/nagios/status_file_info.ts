...
export async function parse_info_block(rl: AsyncIterableIterator<string>): Promise<NagiosStatusInfo> {
  let info: NagiosStatusInfo = {
    created: 0,
    version: "",
    last_update_check: 0,
  ...
  while (!result.done) {
    ...
    // Handles key=value pairs
      ...
      info.last_update_check = Number.parseInt(line.split("last_update_check=")[1]);
    }
